<script>
    ...
    async function transform(widgetConfig, rawData) {
        ...
        for(let i = 0; i < jsonData.length; i++) {
            chartData.labels.push(jsonData[i][0][0].deviceEUI);
        }
    ...
    function createDataSets(dataSetNames, deviceJson, areaColors, chartData) {
        ...
        for (let i = 0; i < dataSetsNumber; i++) {
            ...
            chartData.datasets[i].label = dataSetNames[i];
            
            for(let j = 0 ; j< devicesNumber; j++) {
                chartData.datasets[i].data.push(deviceJson[j][0][i].value);
                chartData.datasets[i].backgroundColor.push(areaColors[j]);
            }
        }
